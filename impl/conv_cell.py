...
class LSTMCell(framework.model.module.AbstractModule):
  ...
  def _build_parameter_graph(self):
    with tf.variable_scope(self.name_scope):
      stddev = 1 / math.sqrt(self._config.dim_hidden + self._config.dim_input)
      self.W = tf.contrib.model_variable('W', 
        shape=(self._config.conv_kernel[0], self._config.conv_kernel[1], self._config.dim_hidden + self._config.dim_input, 4*self._config.dim_hidden),
        dtype=tf.float32,
        initializer=tf.truncated_normal_initializer(stddev=stddev, dtype=tf.float32))
      self._weights.append(self.W)
      self.b = tf.contrib.model_variable('b',
  ...
  def _step(self, input, state, is_training):
    ...
    input = tf.contrib.layers.dropout(input, 
      keep_prob=self._config.keepin_prob, noise_shape=[shape[0], 1, 1, shape[-1]], is_training=is_training)
    c, h = state
    ijfo = tf.nn.conv2d(tf.concat([input, h], -1), self.W, [1, 1, 1, 1], 'SAME') # (None, H, W, 4*dim_hidden)
    ijfo = tf.nn.bias_add(ijfo, self.b)
    i, j, f, o = tf.split(ijfo, 4, axis=3)
    new_c = c * tf.sigmoid(f + 1.0) + tf.sigmoid(i) * tf.tanh(j)
    new_h = tf.tanh(new_c) * tf.sigmoid(o)
    shape = tf.shape(new_h)
    new_h = tf.contrib.layers.dropout(new_h, 
      keep_prob=self._config.keepout_prob, noise_shape=[shape[0], 1, 1, shape[-1]], is_training=is_training)
